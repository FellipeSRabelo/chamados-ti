...
import { PlusCircle, Ticket, Calendar, PlusIcon, Plus } from 'lucide-react';

function ChamadosView({ chamados, setSelectedItem, navigate }) {
  const [tab, setTab] = useState('pendentes');
  const [menuOpen, setMenuOpen] = useState(false);
  
  const listaChamados = chamados.filter(item => 
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', padding: '0 20px 0px 20px' }}>
        ...
        {listaChamados.length === 0 && <div style={{ textAlign: 'center', padding: '40px 0', color: '#94a3b8' }}>Nenhum chamado aqui.</div>}
      </div>
      ...
      <div style={{ position: 'fixed', bottom: '90px', right: '20px', zIndex: '20' }}>
        ...
        <button 
          ...
          style={{
            background: menuOpen ? '#1e293b' : '#1e293b',
            color: 'white',
            border: 'none',
            borderRadius: '50%',
            width: '56px',
